...
def resnet50_model(classes=1000, *args, **kwargs):
    ...
    print(model.summary(), '\n')
    # Compile the model
    model.compile(loss='categorical_crossentropy', optimizer=tf.keras.optimizers.Adam(learning_rate=0.01, clipnorm=0.001),
                  metrics=['accuracy'])
    # Return a model
    return model
...
def train():
    ...
    model.fit(
        ...
        epochs=epochs)
    # Save model to disk
    model.save('C:\\Users\\vladm\\Desktop\\task_firends\\models\\resnet_50.h5')
